...
export const useStandardChat = ({
    ...
    // 在调用主模型之前
    // if (appSettings.isDeepSearchEnabled) {
    //     // 1. 使用 Flash 模型进行“查询扩展”
    //     const queryExpansionPrompt = `用户问题: "${textToUse}"。请将其拆解为 3 个具体的 Google 搜索关键词，以 JSON 数组返回。`;
    //     const expandedQueries = await geminiServiceInstance.generateSuggestions(..., queryExpansionPrompt, ...);keyToUse, userContent, modelContent, language

    //     // 2. 将扩展后的查询词“喂”给主模型，作为上下文提示
    //     // 这样主模型不仅看到用户的问题，还看到了专家的搜索建议，它会更倾向于去搜索这些词
    //     const augmentedPrompt = `
    //     User Question: ${textToUse}

    //     [System Note: Recommended Search Queries to ensure depth]
    //     1. ${expandedQueries[0]}
    //     2. ${expandedQueries[1]}
    //     3. ${expandedQueries[2]}

    //     Please execute searches based on these angles to provide a comprehensive answer.
    //     `;
