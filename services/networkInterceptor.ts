...
export const networkInterceptor = {
    ...
    configure: (enabled: boolean, proxyUrl: string | null) => {
        ...
        let processedProxyUrl = proxyUrl ? proxyUrl.replace(/\/$/, '') : null;

        // Auto-correct localhost to window.location.hostname if accessing from LAN
        if (processedProxyUrl) {
            try {
                const urlObj = new URL(processedProxyUrl);
                const isTargetLocalhost = urlObj.hostname === 'localhost' || urlObj.hostname === '127.0.0.1';
                const isBrowserRemote = window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1';

                if (isTargetLocalhost && isBrowserRemote) {
                    urlObj.hostname = window.location.hostname;
                    processedProxyUrl = urlObj.toString().replace(/\/$/, '');
                }
            } catch (e) {
                // ignore invalid urls
            }
        ...
        currentProxyUrl = processedProxyUrl;
