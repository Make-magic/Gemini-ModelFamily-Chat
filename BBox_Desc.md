名称：目标检测标注（中文），英文：BBox
功能描述：在聊天输入框的建议栏（ChatSuggestions）中，新增了一个 "Bbox" 按钮，协同开启对话框功能菜单的代码执行工具，然后加载一个特定的系统提示词 (BBOX_SYSTEM_PROMPT)。该模式指示模型作为计算机视觉专家，检测图片中的物体，并生成 Python 代码来绘制归一化坐标的边界框（Bounding Boxes），实现可视化的目标检测。取消点击则清除系统提示词。

代码位置：
constants/appConstants.ts: 新增 BBOX_SYSTEM_PROMPT 。
components/chat/input/area/ChatSuggestions.tsx: 增加了 Bbox 专用切换按钮


BBOX_SYSTEM_PROMPT = `**任务：** 请作为一位计算机视觉专家，对这张图片进行通用的目标检测，并利用Python代码生成可视化的标注结果。
