...
import { SlidersHorizontal, Globe, Check, Terminal, Link, X, Telescope, Calculator, Wand2, Sparkles } from 'lucide-react';
...
export const ToolsMenu: React.FC<ToolsMenuProps> = ({
    ...
    disabled, t,
    onToggleCanvasPrompt, isCanvasPromptActive,
    ...
    const menuItems = [
        ...
        { labelKey: isCanvasPromptActive ? 'canvasHelperActive_title' : 'canvasHelperInactive_title', icon: <Wand2 size={18} strokeWidth={2} />, isEnabled: isCanvasPromptActive ?? false, action: () => { if (onToggleCanvasPrompt) handleToggle(onToggleCanvasPrompt); } },
        { labelKey: 'settings_autoCanvasVisualization_label', icon: <Sparkles size={18} strokeWidth={2} />, isEnabled: isAutoCanvasEnabled ?? false, action: () => { if (onToggleAutoCanvas) handleToggle(onToggleAutoCanvas); } },
    ...
    return (
        <div className="flex items-center">
            ...
            {isGoogleSearchEnabled && <ActiveToolBadge label={t('web_search_short')} onRemove={onToggleGoogleSearch} removeAriaLabel="Disable Google Search" icon={<Globe size={14} strokeWidth={2} />} />}
