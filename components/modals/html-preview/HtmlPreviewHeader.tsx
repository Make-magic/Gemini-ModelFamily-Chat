...
export const HtmlPreviewHeader: React.FC<HtmlPreviewHeaderProps> = ({
    ...
    const subtitle = isReactPreview ? 'React App' : t('html_preview');
        ...
        <header className="h-14 px-4 flex items-center justify-between gap-4 bg-[var(--theme-bg-primary)] border-b border-[var(--theme-border-secondary)] z-10 select-none">
            ...
            <div className="flex items-center gap-1">
                ...
                <div className="hidden sm:flex items-center">
                    <button onClick={onZoomOut} className={iconBtnClass} disabled={scale <= minZoom} title={t('zoom_out')}>
                    ...
                    <button onClick={onZoomIn} className={iconBtnClass} disabled={scale >= maxZoom} title={t('zoom_in')}>
                ...
                {/* Action Controls */}
                <button onClick={onRefresh} className={iconBtnClass} title={t('reload')}>
                    <RotateCw size={18} strokeWidth={1.5} />
                </button>
                <button onClick={onDownload} className={iconBtnClass} title={t('download_html')}>
                    <Download size={18} strokeWidth={1.5} />
                </button>
                <button onClick={onScreenshot} className={iconBtnClass} disabled={isScreenshotting} title={t('screenshot')}>
                ...
                <button 
                    ...
                    title={t(isTrueFullscreen ? 'exit_fullscreen' : 'fullscreen')}
                >
                    {isTrueFullscreen ? <Minimize size={18} strokeWidth={1.5} /> : <Expand size={18} strokeWidth={1.5} />}
                </button>
                
                {!isTrueFullscreen && (
                    <button onClick={onClose} className="p-2 text-[var(--theme-text-tertiary)] hover:text-[var(--theme-text-primary)] hover:bg-[var(--theme-bg-danger)]/10 hover:text-[var(--theme-text-danger)] rounded-lg transition-colors ml-1" title={t('close')}>
