...
import { X, ImageIcon, FileCode2, Loader2, Download, FileText, FileDown } from 'lucide-react';
...
interface ExportChatModalProps {
  isOpen: boolean;
  onClose: () => void;
  onExport: (format: 'png' | 'html' | 'txt' | 'md' | 'json' | 'pdf') => void;
                    ...
                    {isLoading ? (
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
                            ...
                            >
                                ...
                                <span className="text-xs text-center text-[var(--theme-text-tertiary)]">A high-resolution image of the entire chat.</span>
                            </button>
                            <button 
                            ...
                            >
                                ...
                                <span className="text-xs text-center text-[var(--theme-text-tertiary)]">A self-contained file with styles.</span>
                            </button>
                            <button 
                                onClick={() => onExport('pdf')}
                                className="flex flex-col items-center justify-center gap-3 p-6 bg-[var(--theme-bg-secondary)] hover:bg-[var(--theme-bg-tertiary)] rounded-lg border border-[var(--theme-border-secondary)] transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[var(--theme-bg-primary)] focus:ring-[var(--theme-border-focus)] transform hover:-translate-y-1 hover:shadow-lg"
                            >
                                <FileDown size={buttonIconSize} className="text-red-500" />
                                <span className="font-semibold text-base text-[var(--theme-text-primary)]">PDF Document</span>
                                <span className="text-xs text-center text-[var(--theme-text-tertiary)]">A clean A4 document perfect for printing.</span>
                            </button>
                            <button 
                            ...
                            >
                                ...
                                <span className="text-xs text-center text-[var(--theme-text-tertiary)]">A formatted text file (.md).</span>
                            </button>
                            <button 
                            ...
                            >
                                ...
                                <span className="text-xs text-center text-[var(--theme-text-tertiary)]">Raw chat data for later import.</span>
                            </button>
                        </div>
                    )}
                </div>
