...
import { SquarePen, Trash2, Pin, PinOff, Download, Copy, Sparkles } from 'lucide-react';
...
export const SessionItemMenu: React.FC<SessionItemMenuProps> = ({ session, menuRef, onStartEdit, onTogglePin, onDuplicate, onGenerateTitle, onExport, onDelete, t }) => (
  <div ref={menuRef} className="absolute right-3 top-9 z-10 w-40 bg-[var(--theme-bg-primary)] border border-[var(--theme-border-secondary)] rounded-md shadow-lg py-1">
    ...
    <button onClick={onGenerateTitle} className="w-full text-left px-3 py-1.5 text-sm text-[var(--theme-text-primary)] hover:bg-[var(--theme-bg-tertiary)] flex items-center gap-2"><Sparkles size={14} /> <span>{t('generate_title', 'Generate Topic Name')}</span></button>
