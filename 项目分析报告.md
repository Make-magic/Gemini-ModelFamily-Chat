...
*   **技术栈**: **UI 框架**React 19 (利用最新的 Hooks 和并发特性)；**语言**TypeScript (类型定义非常详尽), **构建工具**Vite；**样式**Tailwind CSS + 自定义 CSS 变量（支持多主题）；**AI 服务**Google Gemini SDK (`@google/genai`)；**IndexedDB** (通过原生封装，无第三方库依赖，用于存储聊天记录、大文件和设置)；**渲染引擎**: React Markdown, Katex (数学公式), Mermaid & Viz.js (图表), Highlight.js (代码高亮)。
*   **项目结构**:
    *   `components/`: 存放所有 UI 组件，按功能（`chat`, `header`, `settings`）划分。
    *   `hooks/`: 存放自定义 React Hooks，是应用所有业务逻辑的核心抽象层。例如，`useChat.ts` 协调整个聊天功能，`useAppSettings.ts` 管理应用设置。
    *   `services/`: 负责所有外部 API 通信，特别是封装了与 Gemini `Generative` API 和 `Files` API 的交互。`geminiService.ts` 封装了所有与 Gemini API 的交互逻辑。
    *   `backend/`: 包含一个为本地开发设计的、非传统的代理服务器，是“回弹式”代理机制的关键。
